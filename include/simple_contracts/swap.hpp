...
namespace simple_contracts
{
    ...
    class Reserve
    {
        ...
        {
            public:
            auto swap(name owner, const Token& token, uint64_t amount) -> void
            {
                static_cast<typename T::TokenPair*>(this)->swap(token, amount);
            }
        };

        auto get_pair(const Token& token1, const Token& token2) //-> typename T::TokenPair
        {
            return static_cast<T*>(this)->get_pair(token1, token2);
        }
    };

#ifdef CHECKER
    template <typename Base = contract>
    class VariableReserveContractMixin: public Reserve<VariableReserveContractMixin<Base>>, public Base
#else
    template <typename Base, std::enable_if_t<std::is_base_of_v<contract, Base>, int> = 0>
    class VariableReserveContractMixin: public Reserve<VariableReserveContractMixin<Base>>, public Base
#endif
    {
        public:
        using Base::Base;
        using contract_t = VariableReserveContractMixin<Base>;

        TABLE TokenTable_row
        {
            uint64_t id;
            Token    token;
        ...
        using TokenTable = multi_index<name{"token"}, TokenTable_row,
            indexed_by<name{"token"}, const_mem_fun<TokenTable_row, uint128_t, &TokenTable_row::token_key>>
        >;
        using PairTable = multi_index<name{"pair"}, PairTable_row,
            indexed_by<name{"tokenpair"}, const_mem_fun<PairTable_row, uint128_t, &PairTable_row::token_pair_key>>
        >;
        using LiquidityTable = multi_index<name{"liquidity"}, LiquidityTable_row>; 

        TokenTable token_table{Base::get_self(), Base::get_self().value};
        PairTable  pair_table {Base::get_self(), Base::get_self().value};

        private:
        friend class contract_t::TokenPair;
        ...
        auto send(name owner, uint64_t token_id, int64_t amount)
        {
            if(amount > 0)
            {
                ...
                action{
                    permission_level{Base::get_self(), name{"active"}}, 
                    token.get_contract(),
                    name{"transfer"},
                    std::make_tuple(Base::get_self(), owner, asset{amount, token.get_symbol()}, string{})
                }.send();
            }
        }

        public:
        class TokenPair: public Reserve<contract_t>::TokenPair
        {
            private:
            contract_t& outer;
            typename PairTable::const_iterator  it;
            ...
            public:
            TokenPair(contract_t& outer, uint64_t token1_id, uint64_t token2_id): 
                outer(outer), 
                it(
                    outer.pair_table
                    .template get_index<name{"tokenpair"}>()
                    .require_find(token1_id * (static_cast<uint128_t>(std::numeric_limits<uint64_t>::max())+1) + token2_id, "token pair not created")
                )
            {}

            TokenPair(contract_t& outer, uint64_t pair_id): 
                outer(outer), 
                it(
                    outer.pair_table
            ...
            auto withdraw(name owner, uint64_t liquidity_amount) -> void
            {
                ...
                outer.pair_table.modify(
                    it, 
                    same_payer,
                    [amount1=amount1, amount2=amount2, liquidity_amount](PairTable_row& row)
                    {
            ...
            auto swap(name owner, const Token& token, uint64_t amount) -> void
            {
                ...
                if(outer.get_token_id(token)==it->token1_id)
                {
                ...
                else if(outer.get_token_id(token)==it->token2_id)
                {
                    uint64_t token1_reserve = it->token1_reserve, token2_reserve = it->token2_reserve;
        ...
        {
            if(user == Base::get_self()) return; // our tx

            name token_contract = Base::get_first_receiver();
            
            if(memo.empty()) return; // assume donation
            if(memo[0] != 'D') return; // not deposit
        ...
        ACTION deposit(eosio::name owner, uint64_t pair_id)
        {
            ...
            LiquidityTable liquidity_table{Base::get_self(), owner.value};
